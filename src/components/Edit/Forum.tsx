...
export const Forum = () => {
  ...
  // 為狀態數組添加正確的類型定義
  const [editorCards2, setEditorCards2] = useState<ForumCardType[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setEditorContents({
        editor1: data.forumPage[0].section1.dateLabel1,
        editor2: data.forumPage[0].section1.dateLabel2,
      });
      setEditorBackground(data.forumPage[0].section1.background);
      setEditorCards2(data.forumPage[0].section2.card);
    };
