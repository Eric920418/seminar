...
// 定義可更新欄位的 schema
const meetingPageUpdateSchema = z
  .object({
    // 會議頁面各區塊資料
...
const meetingPageMutation = {
  updateMeetingPage: async (_: any, { input }: { input: unknown }) => {
    // 驗證輸入資料
    const parsed = meetingPageUpdateSchema.safeParse(input);
    if (!parsed.success) {
